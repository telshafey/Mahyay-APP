...
import { 
    AppContextType, AppData, Settings, DailyData, AppStats, UserProfile,
...
export const useAppData = (): AppContextType => {
    ...
    const [prayers, setPrayers] = useState<Prayer[]>(PRAYERS_DATA);
    const [nawafil, setNawafil] = useState<Nawafil[]>(NAWAFIL_DATA);
    const [azkarCategories, setAzkarCategories] = useState<AzkarCategory[]>(AZKAR_DATA);
    // For now, FAQs are static, but we can make them dynamic if needed
    ...
    const dailyData = useMemo(() => {
        return appData[todayKey] || createInitialDailyData(prayers, nawafil);
    ...
    const updateDailyData = useCallback((updater: (currentData: DailyData) => DailyData) => {
        setAppData(prev => {
            const currentDayData = prev[todayKey] || createInitialDailyData(prayers, nawafil);
            const updatedDayData = updater(currentDayData);
