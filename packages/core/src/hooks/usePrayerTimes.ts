        ...
        const fetchPrayerTimes = async () => {
            ...
            try {
                ...
                if (!response.ok || !responseText) {
                    throw new Error(`استجابة غير صالحة من الخادم (Status: ${response.status})`);
                }
                ...
                console.error("Data Fetch Error:", message, err);
                
                // Fallback to default Cairo times
                setLocationError(`فشل جلب المواقيت: ${message}. سيتم استخدام مواقيت القاهرة كبديل.`);
